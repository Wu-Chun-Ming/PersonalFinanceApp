...
import { editGoal, fetchGoal } from '@/services/goalService';
import {
  IncomeGoalProps,
  SavingsGoalProps,
  TransactionProps,
...
export const useUpdateGoal = () => {
  return useCustomMutation({
    mutationFn: (updatedGoalsData: {
      savings: SavingsGoalProps;
      income: IncomeGoalProps;
    }) => editGoal(updatedGoalsData),
...
export const useGoalData = (
  ...
  const [incomeProgresses, setIncomeProgresses] = useState({
    day: 0,
    month: 0,
    year: 0,
  });
  ...
  const calculateIncomeGoalProgress = (period: 'day' | 'month' | 'year') => {
    ...
    const lastDate =
      sortedIncomeTransactions[sortedIncomeTransactions.length - 1].date;
  ...
  const getSavingsPerMonth = (selectedYearTransactions: TransactionProps[]) => {
    ...
    const { incomeTotalByMonth, expenseTotalByMonth } =
      selectedYearTransactions.reduce<{
        ...
        (acc, transaction) => {
          ...
          if (transaction.type === TransactionType.INCOME) {
            acc.incomeTotalByMonth[month] =
              (acc.incomeTotalByMonth[month] || 0) + amount;
          } else if (transaction.type === TransactionType.EXPENSE) {
            acc.expenseTotalByMonth[month] =
              (acc.expenseTotalByMonth[month] || 0) + amount;
          }
    ...
    const transactionByMonthArray = months_num.map((month) => ({
      month: month,
      savings:
        (incomeTotalByMonth[month] || 0) - (expenseTotalByMonth[month] || 0),
    }));
  ...
  const getIncomeByPeriod = (
    selectedPeriodIncomeTransactions: TransactionProps[],
    ...
    switch (period) {
      case 'day':
        incomeByPeriod = selectedPeriodIncomeTransactions.reduce<
    ...
    const period_num =
      period === 'day' ? days_num : period === 'year' ? years_num : months_num;
