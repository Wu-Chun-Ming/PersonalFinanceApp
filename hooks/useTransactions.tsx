...
export const useUpdateTransaction = () => {
  return useCustomMutation({
    ...
    }) => editTransaction(id, updatedTransactionData),
  ...
  return useMemo(() => {
    ...
    for (const t of transactions) {
      ...
      if (isNonRecurring && t.date) {
        ...
        if (yearRange && date >= yearRange.start && date < yearRange.end) {
        ...
        if (monthRange && date >= monthRange.start && date < monthRange.end) {
    ...
    const selectedMonthTransactionsData = {
      selectedMonthExpenseTransactions:
        buckets.selectedMonthExpenseTransactions,
    };
...
export const useTransactionSummary = (transactions: TransactionProps[]) => {
  ...
  const percentagesPerCategory = useMemo(() => {
    const percentages: Record<TransactionCategoryType, number> = {} as Record<
      TransactionCategoryType,
      number
    >;
    ...
    for (const category of Object.keys(
      ...
      percentages[category] = grandTotal
        ? (totalsPerCategory[category] / grandTotal) * 100
        : 0;
    }
...
export const useIncomeGraphTransactions = (
  ...
  // 'month' => current year
  // 'year' => last 12 years
  const selectedPeriodIncomeTransactions = useFilteredTransactions(
    incomeTransactions,
    {
      startDate:
        incomeGraphMode === 'day'
          ? new Date(now.getFullYear(), now.getMonth(), 1) // First day of current month
          : incomeGraphMode === 'month'
            ? new Date(now.getFullYear(), 0, 1) // First day of current year
