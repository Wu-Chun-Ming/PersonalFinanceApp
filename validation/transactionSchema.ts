...
export const transactionFilterSchema = Yup.object().shape({
  ...
  type: Yup.string().oneOf(TRANSACTION_TYPES).optional(),
  category: Yup.string().when('type', (transactionType: any, schema) => {
    if (transactionType === TransactionType.EXPENSE) {
      return schema.oneOf(EXPENSE_CATEGORIES, 'Invalid Category').optional();
    }
    if (transactionType === TransactionType.INCOME) {
      return schema.oneOf(INCOME_CATEGORIES, 'Invalid Category').optional();
    }
  ...
  frequency: Yup.string().oneOf(RECURRING_FREQUENCIES).optional(),
});

export const getTransactionSchema = (transactionType: TransactionTypeValue) =>
  Yup.object().shape({
    date: Yup.array()
      .of(Yup.date())
    ...
    category: Yup.string()
      .oneOf(
        getCategoriesByTransactionType(transactionType),
        'Invalid Category',
      )
    ...
    recurring_frequency: Yup.object().shape({
      ...
      time: Yup.object()
        ...
        .test(
          ...
          function (value) {
            const frequency =
              this.options.context?.recurring_frequency?.frequency;
