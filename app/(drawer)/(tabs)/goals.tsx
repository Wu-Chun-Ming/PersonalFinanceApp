...
import { defaultGoalsData, useGoalData, useGoals } from '@/hooks/useGoals';
import {
...
const GoalsScreen = () => {
  const [incomeProgressMode, setIncomeProgressMode] = useState<
    'day' | 'month' | 'year'
  >('month');
  const [incomeGraphMode, setIncomeGraphMode] = useState<
    'day' | 'month' | 'year'
  >('month');
  const { data: goals = defaultGoalsData } = useGoals();
  const {
    data: transactions = [],
  ...
  const { expenseTransactions, incomeTransactions, selectedYearTransactions } =
    useTransactionData(transactions, selectedYear);
  const { selectedPeriodIncomeTransactions } = useIncomeGraphTransactions(
    incomeTransactions,
    incomeGraphMode,
  );
  const {
  ...
  useEffect(() => {
    if (transactions) {
      ...
      const currentSavingsTotal =
        getSavingsPerMonth(transactions)[new Date().getMonth()].savings;
      const currentIncomeTotal = getIncomeByPeriod(incomeTransactions, 'month')[
        new Date().getMonth()
      ].income;
      const currentSavingsRate =
        currentIncomeTotal === 0
          ? 0
          : (currentSavingsTotal / currentIncomeTotal) * 100;
  ...
  const GoalProgress = ({ goalType }: { goalType: 'savings' | 'income' }) => (
    <View
      style={{
        flex: 1,
        alignItems: 'center',
    ...
    >
      ...
      <TouchableNativeFeedback
        onPress={() => {
          setIncomeProgressMode(
            goalType === 'income'
              ? incomeProgressMode === 'day'
                ? 'month'
      ...
      >
        <Progress.Circle
          ...
          progress={
            goalType === 'savings'
              ? savingsProgress
              : incomeProgresses[incomeProgressMode]
          }
          thickness={10}
          showsText={true}
          strokeCap='round'
          ...
          animated={false}
          formatText={(progress) =>
            progress >= 1 ? `Achieved` : `${Math.round(progress * 100)}%`
          }
          textStyle={{
            textAlign: 'center',
          }}
