...
import { BudgetProps, TransactionCategoryType, TransactionType } from '@/types';
...
const BudgetScreen = () => {
  ...
  const { transactionTotalsPerMonth: selectedYearExpenseTotalsPerMonth } =
  ...
  // Budget data
  const { selectedYearBudgets, selectedMonthBudgets } = useBudgetData(
    budgets,
    selectedYear,
    selectedMonth,
  );
  const { budgetTotalsPerMonth: selectedYearBudgetTotalsPerMonth } =
    useBudgetSummary(selectedYearBudgets);

  // Map budgets by category
  const selectedMonthBudgetTotalsByCategory = useMemo(() => {
    const budgetMap: Record<TransactionCategoryType, BudgetProps> =
      {} as Record<TransactionCategoryType, BudgetProps>;
  ...
  const expensesAndBudgetsByMonth = Array.from(
    { length: 12 },
    (_, i) => i + 1,
  ).map((month) => ({
    month: month,
    expensePerMonth:
      selectedYearExpenseTotalsPerMonth[month - 1].expensePerMonth || 0,
    budgetPerMonth:
      selectedYearBudgetTotalsPerMonth[month - 1].budgetPerMonth || 0,
  }));
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          {(selectedYearExpenseTransactions &&
            selectedYearExpenseTransactions.length > 0) ||
      ...
      <View style={styles.centered}>
        ...
        >
          <MonthSelector onMonthChange={(month) => setSelectedMonth(month)} />
        </View>
      </View>
      ...
      <ScrollView>
        ...
        >
          {EXPENSE_CATEGORIES.map((category) => {
            const expenseTotal =
              selectedMonthExpenseTotalsPerCategory[category] || 0;
            ...
            return (
              ...
              >
                ...
                >
                  <HStack
                    key={category}
                    className='flex-1 justify-between max-w-80 w-full'
                  >
