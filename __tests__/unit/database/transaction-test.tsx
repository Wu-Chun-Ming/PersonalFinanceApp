...
describe('Transaction operations', () => {
  ...
  beforeEach(async () => {
    ...
    (
      (databaseModule as any).getDatabaseInstance as jest.Mock
    ).mockResolvedValue(dbInstance);
  ...
  describe('Successful transaction operations', () => {
    ...
    test('should fetch non-recurring transaction successfully', async () => {
      (mockedDb.getFirstAsync as jest.Mock).mockResolvedValue(
        mockDatabaseTransactions[0],
      );

      const response = await showTransaction(1, mockedDb);

      expect(mockedDb.getFirstAsync).toHaveBeenCalledWith(
    ...
    test('should store non-recurring transaction successfully', async () => {
      (mockedDb.runAsync as jest.Mock).mockResolvedValue({
        changes: 1,
        lastInsertRowId: 1,
      });
...
describe('Transaction error handling', () => {
  ...
  test('should throw error when deleting transaction', async () => {
    await expect(destroyTransaction(1, mockedDb)).rejects.toThrow(
      'Error deleting transaction: Database error',
    );
  });
});
