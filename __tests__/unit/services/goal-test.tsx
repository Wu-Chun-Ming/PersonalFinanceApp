...
import { editGoal, fetchGoal, resetGoal } from '@/services/goalService';
...
describe('Goal operations', () => {
  ...
  test('should fetch savings goal successfully', async () => {
    (SecureStore.getItemAsync as jest.Mock).mockResolvedValueOnce(
      mockGoals.savings.date.toISOString(),
    );
    (SecureStore.getItemAsync as jest.Mock).mockResolvedValueOnce(
      mockGoals.savings.amount.toString(),
    );
  ...
  test('should fetch income goals successfully', async () => {
    (SecureStore.getItemAsync as jest.Mock).mockResolvedValueOnce(
      mockGoals.income.perDay.toString(),
    );
    (SecureStore.getItemAsync as jest.Mock).mockResolvedValueOnce(
      mockGoals.income.perMonth.toString(),
    );
    (SecureStore.getItemAsync as jest.Mock).mockResolvedValueOnce(
      mockGoals.income.perYear.toString(),
    );
  ...
  test('should update goals successfully', async () => {
    ...
    expect(SecureStore.setItemAsync).toHaveBeenNthCalledWith(
      1,
      'savingsGoalDate',
      dayjs(mockGoals.savings.date).format('YYYY-MM-DD'),
    );
    expect(SecureStore.setItemAsync).toHaveBeenNthCalledWith(
      2,
      'savingsGoalAmount',
...
describe('Goal error handling', () => {
  ...
  test('should throw error when fetching invalid goal', async () => {
    await expect(fetchGoal('invalid' as any)).rejects.toThrow(
      'Invalid goal type',
    );
  });

  test('should throw error when resetting invalid goal', async () => {
    await expect(resetGoal('invalid' as any)).rejects.toThrow(
      'Invalid goal type',
    );
  });
  ...
  test('should throw error when editing goal', async () => {
    ...
    await expect(editGoal(mockGoals)).rejects.toThrow(
      'Error updating goal: SecureStore error',
    );
  });
});
